...
public class CommandLineDictionary : Dictionary<string, string>
{
    ...
    public override string ToString()
    {
        ...
        foreach (KeyValuePair<string, string> pair in this)
        {
            ...
            {
                // There is no value, so we just serialize the key
    ...
    private void AddArgument(string argument)
    {
        ...
        if (argument.StartsWith(KeyCharacter.ToString(), StringComparison.OrdinalIgnoreCase))
        {
            ...
            value = splitArg.Length > 1 ? string.Join("=", splitArg, 1, splitArg.Length - 1) : string.Empty;
        }
        else
        {
            throw new ArgumentException("Unsupported value line argument format. Should start with " + KeyCharacter, argument);
        }
